...
same as `draw_and_load`. A supplied `sample id` binds an untagged slot to the
preloaded vial; an existing different tag is refused. No mixing is performed
and no timestamp ID is invented. The Automation tab's **Draw and
...
Operations serialize per pump server. Mixing-server cleanup can overlap a
flowcell-server draw; operations on the same server wait for each other.
Each keeps the `(ok, detail)` contract.
...
`operation_active` goes false before returning. Mixing also requires an explicit
...
and write a value, recorded as `advisory_speed_rpm`, and out-of-range is
...
PALmixer retains the slot's prepared sample ID; only an untagged slot gets a
new ID after a successful mix. A missing PVapp sample is never created with
incomplete composition data; its mixing record remains pending locally.

Each record contains:

- A unique `run_id`, sample ID, carousel ID, slot and selected flowcell.
- `pump.settings`: the complete `parameters` snapshot returned when the pump
  dashboard accepts `sample_make`, including per-channel speeds (mL/min),
  volumes (uL), start delays (ms), aspiration/correction settings and TTL timing.
  Later dashboard edits cannot change this snapshot. An older dashboard that
  supplies no snapshot is recorded with `settings: null` and source `unavailable`.
- UTC request/acceptance/end observations, elapsed seconds, ordered workflow
  step observations, and the pump's terminal operation text. These describe the
  whole pump operation, including aspiration and corrections; they are not
  measured liquid-contact times. The request ID is generated by PALmixer, not
  a durable job ID assigned by the pump service.
- Separate `mixing_status` and `workflow_status`: a completed mix remains
  completed if later transport fails. STOP/abort, failure and unknown outcomes
  are retained. Workflow completion does not assert that background cleaning or
  ongoing shaking has finished. Simulation records remain local.

The accepted speeds, including controller-quantized `effective_*` values, and
TTL timing are **planned settings**, not measured flow or electrical readback.
The advisory RPM value is stored separately. Manual standalone pump actions
outside `make_sample` are not attributed to a sample automatically.

Records are atomically saved in `json/mixing_records/` (ignored by Git), including
the accepted snapshot before waiting for pump completion. Set
`PALMIXER_MIXING_RECORDS_DIR` to use another persistent directory. A background
worker retries PVapp delivery after a workflow and every 30 seconds. Its local
`pvapp_sync` metadata shows `pending`, `synced` or `local_only`, with the last
error when relevant. Keep pending files for retry; startup marks unfinished
workflows `interrupted`, with unknown mixing outcomes where completion was not
observed. It does not rerun hardware operations.

Delivery merges existing PVapp data and upserts by run ID, so retries do not
duplicate runs or remove composition. Legacy `mixed_*` fields summarize the
latest confirmed completed mix. PVapp's GET/PUT update is not atomic: concurrent
writers still need a server-side event-append API or revision check. Credentials
use the existing PVapp configuration / `set_credentials` mechanism.

Restart PALmixer **when idle** to load this implementation. The deployed pump
dashboard must also support the accepted-parameters response to populate settings.
This change does not upload scattering data; the self-driving campaign currently
stores scans/curves in its results Zarr and associated files and, with PVapp
enabled, now appends measurement references and analysis summaries itself.

### Correlated completion for self-driving

`get_state` advertises `action_result_version: 1`, `busy`, `current_action_id`
and `simulated`. Worker commands return `ACCEPTED action_id=<id>`; the matching
`last_result` contains `action_id`, the original normalized `command`, `ok`,
UTC `finished_at`, and a tracked-state snapshot at the end. Both success and
failure return to idle, so clients must check the matching result.

`draw_load_sample <slot> <sample_id>` now binds an untagged preloaded vial to
the supplied ID before drawing and refuses an existing conflicting ID. It
does not mix or invent a mixing record. The flowcell pump must explicitly
report that the selected channel completed `draw_to_flowcell`; idle after an
error or STOP is not successful loading.

The updated self-driving client requires this completion protocol before
worker actions and verifies the sample tag and selected flowcell at the beam
before measurement. Update both repositories and restart the PALmixer server
when idle. Existing GUI clients that accept replies starting with `ACCEPTED`
remain compatible. See the self-driving repository's
[sample provenance guide](../aps12selfdriving/docs/sample-provenance.md).
